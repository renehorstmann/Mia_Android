...
//@@@USE_BILLING@@@    private static volatile boolean billing_loaded = false;
    ...
    private static native void oFetchProgress(long fetch_data, int uploaded, int downloaded, int upload_total, int download_total);
...
public static Object[] oEventFetchBlocking(long opt_fetch_data, String url, String method, String[] opt_headers, byte[] optPostData) {
        Log.d(TAG, "oEventFetchBlocking " + method + " request: " + url + " progress?: " + String.valueOf(opt_fetch_data!=0));

        HttpURLConnection urlConnection = null;
        Object[] result = null;


        int progress_uploaded = 0;
        int progress_downloaded = 0;
        int progress_upload_total = 0;
        int progress_download_total = 0;

        int chunk_size = 8192;
        int progress_delay = 16;

        int progress_cnt = 0;

        if(optPostData != null) {
            progress_upload_total = optPostData.length;
        }
        ...
        try {
            ...
            if(opt_headers != null) {
                int headers = opt_headers.length/2;
                for(int i=0; i<headers; i++) {
                    urlConnection.setRequestProperty(opt_headers[i*2], opt_headers[i*2+1]);
                }
            }
            ...
            if (optPostData != null) {
                ...
                urlConnection.setFixedLengthStreamingMode(optPostData.length);

                OutputStream os = urlConnection.getOutputStream();
                int offset = 0;
                while (offset < optPostData.length) {
                    int toWrite = Math.min(chunk_size, optPostData.length - offset);
                    os.write(optPostData, offset, toWrite);
                    offset += toWrite;

                    if(progress_cnt <=0 && opt_fetch_data!=0) {
                        progress_cnt = progress_delay;
                        progress_uploaded = offset;
                        oFetchProgress(opt_fetch_data, progress_uploaded, progress_downloaded, progress_upload_total, progress_download_total);
                    }
                    progress_cnt--;
                }
                os.close();
            ...
            progress_download_total = urlConnection.getContentLength();
            ...
            for (int i = 0; ; i++) {
                ...
                if (key != null && value != null) {
                    serverHeader.append(key);
                    serverHeader.append(": ");
                    serverHeader.append(value);
                    serverHeader.append("\r\n");
                }
            }
            ...
            result = new Object[3];
            result[0] = Integer.valueOf(responseCode);
            result[1] = serverHeader.toString();
            result[2] = byteArrayOutputStream.toByteArray();

            byteArrayOutputStream.close();


        } catch (Exception e) {
            Log.e(TAG, "oEventFetchBlocking failed: " + e.toString());
        }
    ...
    public static void aRumble(float intensity, float time) {
        if (aRumble_vibrator == null) {
            aRumble_vibrator = (Vibrator) getContext().getSystemService(Context.VIBRATOR_SERVICE);
            if (aRumble_vibrator == null) {
                return;
            }
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // Pre-Oreo: no amplitude control, just duration
            aRumble_vibrator.vibrate(duration_ms);
        }
    }
